...
// $Id: package-release-nodes.php,v 1.1.2.26 2007/01/19 09:57:54 dww Exp $
// $Name:  $
...
function verify_packages($task) {
  ...
  foreach ($releases as $release) {
    ...
    $variables = array();
    $variables['%file'] = theme('placeholder', $file_path);
    if ($real_hash != $db_hash) {
      $valid_hash = FALSE;
      $num_wrong_hash++;
      $variables['@db_hash'] = check_plain($db_hash);
      $variables['@real_hash'] = check_plain($real_hash);
    }
